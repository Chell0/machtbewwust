...
async function fetchMaterialSectionMaterials() {
    const query = `
    *[_type == "material_section" && title == "Bildungsmaterial"] {
    title,
    ...
    const material_sections = await client.fetch(query);
    if (material_sections.length > 0) {
        return material_sections[0].materials;
    }
    return [];
}
